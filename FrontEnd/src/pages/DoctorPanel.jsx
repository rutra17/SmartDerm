...
import { getConsultas, getMensagens, atualizarConsulta } from '../services/api';
...
function DoctorPanel() {
    ...
    const abrirTriagem = async (consulta) => {
        ...
        const data = await getMensagens(consulta.id);
        ...
        if (Array.isArray(data)) {
            ...
            data.forEach(msg => {
                if (msg.imagem_url) imagensEncontradas.push(msg.imagem_url);
                if (msg.role === 'assistant' && msg.texto) analiseEncontrada = msg.texto;
            });

            setDadosTriagem({ imagens: imagensEncontradas, analiseIA: analiseEncontrada });
        }
        setCarregando(false);
    };
    ...
    const guardarLaudoMedico = async () => {
        ...
        const data = await atualizarConsulta(consultaSelecionada.id, {
            laudo_medico: laudoFinal,
            status: 'finalizada',
        });

        if (data.error) {
            alert("Ocorreu um erro ao guardar o laudo.");
            setCarregando(false);
            return;
        }

        setConsultaSelecionada(prev => ({ ...prev, status: 'finalizada', laudo_medico: laudoFinal }));
        alert(`✅ Laudo guardado com sucesso!`);
        buscarFilaDePacientes();
        setCarregando(false);
    };
                            ...
                            >
                                ...
                                <div className="text-xs text-gray-400 mt-1 flex justify-between items-center">
                                    ...
                                    {consulta.status === 'finalizada' ? (
                                        <span className="bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded text-[10px] font-bold">FINALIZADA</span>
                                    ) : (
                                        <span className="bg-orange-500/20 text-orange-400 px-2 py-0.5 rounded text-[10px] font-bold">PENDENTE</span>
                                    )}
                                </div>
                            </button>
                        ))
                    ...
                    <div className="max-w-4xl mx-auto w-full animate-fade-in space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="bg-[#202123] rounded-xl border border-gray-600 p-4 flex flex-col">
                                ...
                                <div className="flex-1 bg-[#343541] p-4 rounded-lg text-sm text-gray-300 overflow-y-auto max-h-[400px] markdown-formatado">
                                    <ReactMarkdown>{dadosTriagem.analiseIA}</ReactMarkdown>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-[#202123] rounded-xl border border-blue-500/50 p-4 mt-8">
                            ...
                            <div className="flex justify-end">
                                ...
                                >
                                    {consultaSelecionada.status === 'finalizada' ? 'Atualizar Laudo' : 'Guardar Laudo e Finalizar'}
                                </button>
                            </div>
                        </div>
                    </div>
